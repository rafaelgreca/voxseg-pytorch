...
def test1():
    ...
    utils.save(feats, os.path.join(os.getcwd(), "tests", "files", "features", "feats.h5"))
...
def test2():
    ...
    feats = pd.read_hdf(os.path.join(os.getcwd(), "tests", "files", "features", "feats.h5"))
    ...
    assert ("start" in endpoints.columns.tolist()) and ("end" in endpoints.columns.tolist())
    assert (len(endpoints["start"].values.tolist()) > 0) and (len(endpoints["end"].values.tolist()) > 0)
    
    run_cnnlstm.to_data_dir(endpoints, os.path.join(os.getcwd(), "tests", "files", "output"))
