...
def decode(
    ...
    if targets["predicted-targets"].iloc[0].shape[-1] == 4:
        ...
        temp = pd.concat(
            [
                _targets_to_endpoints(
                    medfilt([0 if (j * prior).argmax() == 1 else 1 for j in i], filt),
                    0.32,
                )
                for i in targets["predicted-targets"]
            ],
            ignore_index=True,
        )
    elif targets["predicted-targets"].iloc[0].shape[-1] == 2:
        prior = np.array([speech_thresh, 1 - speech_thresh])
        temp = pd.concat(
            [
                _targets_to_endpoints(
                    medfilt([0 if (j * prior).argmax() == 0 else 1 for j in i], filt),
                    0.32,
                )
                for i in targets["predicted-targets"]
            ],
        ...
        print(
            f'ERROR: model provided has {targets["predicted-targets"].iloc[0].shape[-1]} outputs. Model expected to have 2 or 4 outputs.'
        )
    ...
    targets["utterance-id"] = (
        ...
        + "_"
        + ((targets["start"] * 100).astype(int)).astype(str).str.zfill(7)
        + "_"
...
def to_data_dir(endpoints: pd.DataFrame, out_dir: str) -> None:
    ...
    pd.concat(
        [
            endpoints[["utterance-id", "recording-id"]],
            endpoints[["start", "end"]].astype(float).round(3),
        ],
        axis=1,
...
if __name__ == "__main__":
    ...
    if args.model_path is not None:
        ...
        model = models.load_model(
            f"{os.path.dirname(os.path.realpath(__file__))}/models/cnn_bilstm.h5"
        )
