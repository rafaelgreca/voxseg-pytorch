...
def print_confusion_matrix(scores: Dict[str, Dict[str, int]]) -> None:
    ...
    print(
        ...
        + "\t\t\t\tSpeech\tNon-speech\n"
        + "Predicted\tSpeech\t\t"
        + str(round(tp / (tp + fn), 3))
        + "\t"
        + str(round(fp / (tn + fp), 3))
        + "\n"
        + "\t\tNon-speech\t"
        + str(round(fn / (tp + fn), 3))
        + "\t"
        + str(round(tn / (tn + fp), 3))
    )


def score(
    wav_scp: pd.DataFrame,
    sys_segs: pd.DataFrame,
    ref_segs: pd.DataFrame,
    wav_segs: pd.DataFrame = None,
...
def _segments_to_mask(
    wav_scp: pd.DataFrame, segments: pd.DataFrame, frame_length: float = 0.01
) -> Dict[str, np.ndarray]:
    """
    ...
    wav_scp["duration"] = (
        wav_scp["extended filename"]
        .apply(lambda x: len(wavfile.read(x)[1]) / wavfile.read(x)[0])
        .astype(float)
    )
    wav_scp["mask"] = (
        round(wav_scp["duration"] / frame_length).astype(int).apply(np.zeros)
    )
    segments["frames"] = (
        round(segments["end"] / frame_length).astype(int)
        - round(segments["start"] / frame_length).astype(int)
    ).apply(np.ones)
    temp = wav_scp.merge(segments, how="left", on="recording-id")
    for n, _ in enumerate(temp["mask"]):
        if not np.isnan(temp["start"][n]):
            temp["mask"][n][
                round(temp["start"][n] / frame_length) : round(
                    temp["end"][n] / frame_length
                )
            ] = temp["frames"][n]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="evaluate.py", description="Evaluate the performance of VAD model output."
    )
