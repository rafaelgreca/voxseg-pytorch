...
def extract(
    data: pd.DataFrame, frame_length: float = 0.32, nfilt: int = 32, rate: int = 16000
) -> pd.DataFrame:
    ...
    data["features"] = data.apply(
        lambda x: _calculate_feats(x, frame_length, nfilt, rate), axis=1
    )
...
def _calculate_feats(
    row: pd.DataFrame, frame_length: float, nfilt: int, rate: int
) -> np.ndarray:
    ...
    try:
        assert (
            len(
                range(
                    0,
                    int(len(sig) - 1 - (frame_length + 0.01) * rate),
                    int(frame_length * rate),
                )
            )
            > 0
        )
        feats = []
        for j in utils.progressbar(
            range(
                0,
                int(len(sig) - 1 - (frame_length + 0.01) * rate),
                int(frame_length * rate),
            ...
            feats.append(
                np.flipud(
                    logfbank(
                        sig[j : int(j + (frame_length + 0.01) * rate)],
                        rate,
                        nfilt=nfilt,
                    ).T
                )
