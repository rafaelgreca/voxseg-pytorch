...
def get_labels(
    ...
    data["labels"] = data.apply(
        lambda x: _generate_label_sequence(x, frame_length, rate), axis=1
    )
...
def prep_data(path: str) -> pd.DataFrame:
    ...
    assert (
        utt2spk is not None and segments is not None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="prep_labels.py", description="Prepare labels for model training."
    )
